...
post '/' do
  ...
  if (request.type == 'LAUNCH_REQUEST')
    ...
    response.add_speech('Drug User is running!')
    response.add_hash_card( { title: 'Drug User Run', subtitle: 'Drug User Running!' } )
  end
  ...
  if (request.type == 'INTENT_REQUEST')
    ...
    response.add_hash_card( { title: 'Drug User Intent', subtitle: "Intent #{request.name}" } )
  end
