...
post '/' do
  ...
  if (request.type == 'LAUNCH_REQUEST')
    ...
    response.add_speech('Silver Surfer is running!')
    response.add_hash_card( { title: 'Silver Surfer Run', subtitle: 'Silver Surfer Running!' } )
  end
  ...
  if (request.type == 'INTENT_REQUEST')
    ...
    response.add_hash_card( { title: 'Silver Surfer Intent', subtitle: "Intent #{request.name}" } )
  end
